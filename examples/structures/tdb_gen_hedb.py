...
# Optional settings keyed by an order-independent system name such as CrHf.
# Each override may define terms_in, mult_in, sublattice_map, and
# fixed_compositions using the same shapes as the global settings above.
...
if "phases" not in globals():
    ...
    if not _active_d:
        raise ValueError(
            "No lattice parameters are available for the selected elements; "
            "provide phases directly or use full_framework.toml averaging settings"
        )
    _avg_a = sum(_DIBORIDE_A[el] for el in _active_d) / len(_active_d)
    _avg_c = sum(_DIBORIDE_C[el] for el in _active_d) / len(_active_d)
    print(f"Diboride lattice estimate: a={_avg_a:.4f} Å  c={_avg_c:.4f} Å  " f"(avg of {_active_d})")

    # Direct runs retain the historical HEDB prototype. The combined runner
    # injects a generalized phase definition from TOML instead.
...
if run_tdb:
    ...
    def _make_generator(compositions, override=None):
        ...
        return BladeTDBGen(
            ...
            terms_in=_merged(terms_in, override.get("terms_in")) or None,
            mult_in=_merged(mult_in, override.get("mult_in")) or None,
            sublattice_map=(_merged(sublattice_map, override.get("sublattice_map")) or None),
            tdb_params=tdb_params,
            fixed_compositions=(_merged(fixed_compositions, override.get("fixed_compositions")) or None),
        )

    if system_overrides:
        for composition in composition_list:
            system_name = "".join(sorted(composition))
            override = system_overrides.get(system_name, {})
            print(f"TDB system {system_name}: " f"{'using override' if override else 'using global settings'}")
            _make_generator([composition], override).fit()
    else:
        _make_generator(composition_list).fit()
