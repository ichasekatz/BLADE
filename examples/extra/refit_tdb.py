...
fixed_elements  = {"B"}          # fixed sublattice species (excluded from plot axes)
metal_fraction  = 1 / 3          # fraction of atoms that are metal (1/3 for MB2)
...
comp_dirs = sorted(
    [d for d in comps_dir.iterdir() if d.is_dir()]
    if compositions is None
    else [comps_dir / c for c in compositions]
)
...
for comp_dir in comp_dirs:
    ...
    try:
        ...
        if tdbs:
            ...
            print(f"  Warning: no TDB file found after -tdb")
        ...
        if update_plots and tdbs:
            ...
            for tdb_path in tdbs:
                ...
                metals = sorted(el.title() for el in all_els
                                if el.upper() not in {f.upper() for f in fixed_elements})
                fixed_sp = {el.title(): (1.0 - metal_fraction) / max(len(fixed_elements), 1)
                ...
                viz.plot_gibbs_energy(
                    tdb=tdb, metals=metals, phase=phases[0],
                    fixed_species=fixed_sp, temperatures=plot_temps,
                ...
                if len(metals) == 2:
                    viz.plot_binary_phase_diagram(
                        tdb=tdb, metals=metals, phases=phases,
